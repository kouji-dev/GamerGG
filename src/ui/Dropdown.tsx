import React, {FC, InputHTMLAttributes, MouseEventHandler, PropsWithChildren, ReactNode, useRef, useState} from "react";
...
export const Dropdown: FC<PropsWithChildren<DropdownProps>> = (props) => {
    ...
    const content = open ? <div className={contentCls} style={{...coords, minWidth: window.getComputedStyle(containerRef.current).width}}>{children}</div> : null;
    const arrow = open ? <KeyboardArrowUp className='!text-floral'/> : <KeyboardArrowDown className='!text-floral'/>;
